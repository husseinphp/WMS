...
<h1 class="text-center mt-3">  طباعة السند التالف  </h1>
...
<!-- <form action="damaged.php" method="post"> -->
	...
	<!-- <button class="btn btn-primary" name="searchbtn"> بحث    </button> <input type="" name="searchinput" class="w-25 bg-white" placeholder="رقم الصفحة في سجل المخزن"> -->
...
if (isset($_GET['iditems'])) {
 ...
 $id = $_GET['iditems'];
	...
	$sql22='SELECT * FROM damaged WHERE id ='.$id;

        $res22=$con->query($sql22);
        $row=$res22->fetch_assoc();
            // echo $row["name"];
...
<input type="hidden" name="id"  class="w-50" value="<?php echo $row["id"];?>">
	<div class="col"><p>  أسم المادة </p>  <input type="text" name="name"  class="w-50" value=" <?php echo $row["name"];?> "></div>
	<div class="col"><p>  العدد </p>  <input type="text" name="count"  class="w-50" value=" <?php echo $row["count"];?>" ></div>
	<div class="col"><p>  التاريخ  </p>  <input type="text" name="date"  class="w-50" value=" <?php echo $row["date"];?>" ></div>
...
<div class="row text-center mt-5 mb-5" dir="rtl" lang="ar" > 
	...
	<div class="col"><p> الصفحة  </p>  <input type="text" name="page" class="w-50" value=" <?php echo $row["page"];?>" ></div>
	<div class="col"><p>  جهه الاستهلاك   </p>  <input type="text" name="consumption"  class="w-50" value=" <?php echo $row["consumption"];?>"></div>
	<div class="col"><p>  التسلل </p>  <input type="text" name="sequence"  class="w-50" value=" <?php echo $row["sequence"];?>"></div>


	<div class="col-12 text-center mt-5">
		<button class="btn btn-success w-25 " name="update-itemsxxx" onclick="window.print();" target="_blank" style="cursor:pointer;"  > طباعة </button>
	</div>
...
// UPDATE materials SET name = 'xxxx' WHERE name LIKE
			...
			// $sql2= "UPDATE damaged  SET name = number_input - ".$count." WHERE name LIKE '%".$SearchInDataBaseByName."%'" ;

		// $sql2= "UPDATE materials  SET number_input = number_input - ".$count." WHERE name LIKE '%".$SearchInDataBaseByName."%'";

		$sql2 = "UPDATE damaged  SET name = '$name' , sequence = '$sequence' , count = '$count' , consumption  = '$consumption'  , date = '$date' , page = '$page'  WHERE id = '$id' " ;
...
include 'tempalte/footer.php';
 ...
 ?>
	...
	<!-- <div class="col"><p>  </p>  <input type="text" name=""></div> -->
